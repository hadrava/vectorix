...
v_image vectorizer_example::vectorize(const pnm_image &image) { // This is *not* a vectorizer. It just produce fixed vector image with same size as input
	...
	out.add_line(line);
	offset convertor(out, *par);
	convertor.convert_to_outline(line); // Convert from stroke to fill
