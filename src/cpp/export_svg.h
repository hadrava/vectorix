...
private:
	...
	static void svg_write_header(FILE *fd, const v_image &image) { // Write image header
		...
		if (!global_params.output.svg_underlay_image.empty()) // Original (or other) image can be linked and displayed in background, needs xlink extension
		...
		if (!global_params.output.svg_underlay_image.empty()) {
			fprintf(fd, "    <image\n");
			fprintf(fd, "       y=\"0\" x=\"0\"\n");
			fprintf(fd, "       xlink:href=\"file://%s\"\n", global_params.output.svg_underlay_image.c_str()); // Display background image
			fprintf(fd, "       width=\"%f\"\n", image.width);
			fprintf(fd, "       height=\"%f\" />\n", image.height);
		}
	};
