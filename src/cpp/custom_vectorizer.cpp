...
uchar custom::nullpixel;

uchar &custom::safeat(const Mat &image, int i, int j) {
...
void custom::vectorize_imshow(const string& winname, InputArray mat) {
	return imshow(winname, mat);
}
int custom::vectorize_waitKey(int delay) {
...
void custom::pridej(Mat &out, Mat &bw, int iterace) {
...
void custom::normalize(Mat &out, int max) {
	for (int i = 0; i < out.rows; i++) {
		for (int j = 0; j < out.cols; j++) {
...
void custom::my_threshold(Mat &out) {
	for (int i = 0; i < out.rows; i++) {
		for (int j = 0; j < out.cols; j++) {
...
Point custom::find_adj(const Mat &out, Point pos) {
	int max = 0;
	int max_x = -1;
	int max_y = -1;
...
v_line *custom::trace(Mat &out, const Mat &orig, Mat &seg, Point pos) {
...
void custom::my_segment(Mat &out, const Mat &orig, Mat &seg, v_image &vect) {
	double max;
	Point max_pos;
...
v_image custom::vectorize(const pnm_image &original) {
